...
export function getLivePrice(symbol: string): number {
  ...
  return parseFloat((base + variation).toFixed(symbol.includes("JPY") ? 3 : symbol === "XAUUSD" ? 2 : 5));
}
...
export async function generateSignalFromAI(pair: string, currentPrice: number): Promise<Signal> {
  ...
  const prompt = `You are an expert Forex analyst using SMC (Smart Money Concepts) and ICT (Inner Circle Trader) methodologies. Analyze ${pair} at price ${currentPrice} during the ${session} session.

Generate a swing trading signal. Return ONLY raw JSON — no markdown, no backticks, no explanation:

{"direction":"BUY","strength":"STRONG","entry":${currentPrice},"stopLoss":0,"takeProfit1":0,"takeProfit2":0,"takeProfit3":0,"riskReward":2.5,"timeframe":"H1","smcConcepts":["Order Block","BOS"],"ictConcepts":["Optimal Trade Entry"],"marketStructure":"Bullish","confluence":78,"analysis":"2-3 sentence technical analysis using SMC/ICT concepts specific to ${pair}.","bias":"One sentence directional bias.","keyLevels":[{"label":"Demand OB","price":0,"type":"ob"},{"label":"FVG","price":0,"type":"fvg"}],"sessionContext":"${session}"}

Rules:
- direction: "BUY", "SELL", or "WAIT"
- strength: "STRONG", "MODERATE", or "WEAK"
- entry near ${currentPrice}
- stopLoss behind structure (25-40 pips for majors, 150-300 for XAUUSD)
- takeProfit1 at 1.5x risk, takeProfit2 at 2.5x risk, takeProfit3 at 4x risk
...
- confluence: 0-100
- keyLevels: 2-3 realistic price levels near ${currentPrice}`;
      ...
      const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are a professional Forex analyst specializing in SMC and ICT concepts. Always respond with raw JSON only — no markdown, no backticks.",
            },
...
function generateFallbackSignal(pair: string, price: number, session: string, timestamp: string): Signal {
  ...
  return {
    ...
    analysis: `${pair} showing a ${isBuy ? "bullish" : "bearish"} order block confluence on H1 timeframe. Price has swept ${isBuy ? "sell-side" : "buy-side"} liquidity and is returning to ${isBuy ? "discount" : "premium"} pricing zone. M15 CHoCH confirms ${isBuy ? "bullish" : "bearish"} continuation into the setup.`,
    bias: `${isBuy ? "Bullish" : "Bearish"} bias — ${isBuy ? "targeting higher liquidity pools above" : "targeting lower liquidity pools below"} in line with H4 structure`,
    keyLevels: [
